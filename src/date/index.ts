...
server.tool(
  "set_timezone",
  "Set the default timezone for date resolution",
  {
    timezone: z
      .string()
      .describe("The timezone to set as default (e.g., 'UTC', 'Asia/Tokyo')"),
  },
  async (params) => {
    try {
      dateService.setTimezone(params.timezone);
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify({
              success: true,
              message: `Default timezone set to ${params.timezone}`,
            }),
          },
        ],
      };
